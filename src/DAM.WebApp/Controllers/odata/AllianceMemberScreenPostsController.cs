...
namespace DAM.WebApp.Controllers.odata
{
    ...
    public class AllianceMemberScreenPostsController : ODataController
    {
        private readonly IAllianceManagementServiceAsync _allianceService;
        private readonly AllianceContext _dbContext;
        public AllianceMemberScreenPostsController(IAllianceManagementServiceAsync allianceService,
        ...
        [EnableQuery, HttpGet]
        public async Task<IQueryable<AllianceMember_ScreenPost>> Get(ulong allianceid)
        {
            ...
            return _dbContext.Member_ScreenPosts
                .Join(_dbContext.Members, msp => msp.AllianceMemberId, m => m.Id, (msp, m) => new { msp = msp, m = m })
                .Where(mmsp => mmsp.m.AllianceId == alliance.Id)
                .Select(r => r.msp);
        }
    }
}
